...
namespace ESPlus.Wyrm
{
    public class WyrmDriver : IWyrmDriver
    {
        ...
        private async Task<TcpClient> CreateAsync()
        {
            ...
            // var host = (await Dns.GetHostAddressesAsync(_host)).FirstOrDefault(ip => ip.AddressFamily == AddressFamily.InterNetwork)?.ToString();
        ...
        public async IAsyncEnumerable<WyrmEvent2> EnumerateStream(string streamName, [EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                yield return await ReadEventAsync(stream, length - sizeof(Int32), cancellationToken, f => true);
            }
        }

        private Task<WyrmEvent2> ReadEventAsync(NetworkStream reader, int length,
            CancellationToken cancellationToken, Func<string, bool> filter)
        {
            return ReadEventAsyncBack(reader, length, cancellationToken, filter);
            // var createEvent = "";
            // var payload = await reader.ReadBinaryAsync(length, cancellationToken);
            // var segment = payload.AsMemorySegment();
            // var result = new WyrmEvent2
            // {
            //     Position = segment.SliceAt(32).ToArray(),
            //     Offset = segment.To<long>(),
            //     TotalOffset = segment.To<long>(),
            //     IsAhead = segment.To<bool>(),
            //     EventId = new Guid(segment.SliceAt(16).AsSpan()),
            //     Version = segment.To<long>(),
            // };
            //
            // var uncompressedSize = segment.To<int>();
            // var compressedSize = segment.To<int>();
            // var encryptedSize = segment.To<int>();
            // var clock = segment.To<long>();
            // var ms = segment.To<long>();
            // var eventTypeLength = segment.To<int>();
            // var streamNameLength = segment.To<int>();
            // var metaDataLength = segment.To<int>();
            // var payloadLength = segment.To<int>();
            // var streamName = segment.SliceAt(streamNameLength).ToString();
            //     
            ...
            // }
            //
            // var eventType = segment.SliceAt(eventTypeLength).ToString();
            // var time = DateTimeOffset.FromUnixTimeSeconds(clock).AddTicks(ms * 10);
            // var compressed = segment.SliceAt(compressedSize).ToArray();
            ...
            //
            // if (eventType == "Wyrm.StreamDeleted")
            // {
            //     var createEventLength = segment.To<int>();
            //     createEvent = segment.SliceAt(createEventLength).ToString();
            // }
            //
            // Array.Copy(uncompressed, metadata, metadata.Length);
            // Array.Copy(uncompressed, metadata.Length, data, 0, data.Length);
            //
            // result.TimestampUtc = time.DateTime;
        ...
        private async Task<WyrmEvent2> ReadEventAsyncBack(NetworkStream reader, int length,
            CancellationToken cancellationToken, Func<string, bool> filter)
        {
            ...
            [EnumeratorCancellation] CancellationToken cancellationToken)
        {
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                yield return await ReadEventAsync(stream, length - sizeof(Int32), shortLivedToken.Token, f => true);
            }
        }
        ...
        public async IAsyncEnumerable<WyrmEvent2> EnumerateAllByStreamsAsync(Func<string, bool> func,
            [EnumeratorCancellation] CancellationToken cancellationToken, params Type[] filters)
        {
            var algorithm = xxHashFactory.Instance.Create(new xxHashConfig { HashSizeInBits = 64 });
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                var evt = await ReadEventAsync(stream, length - sizeof(Int32), cancellationToken, func);
